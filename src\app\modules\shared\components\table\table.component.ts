...
export class TableComponent implements OnInit{
  ...
  primaryKey=''
  disableArchive=true
  constructor(private sharedService: SharedService,private toast:ToastrService,private ngxUiLoader: NgxUiLoaderService,private logger: NGXLogger){}
  ...
  getTableData(){
    ...
    if(this.page){
      ...
      this.getTableSubscription = this.sharedService.getCrudData(this.page,params).subscribe({
        next: (res)=>{
          this.logger.log(`${this.pageName} table loaded successfully.`)
        ...
        error: (error)=>{
          this.logger.error(`${this.pageName} table load failed.`,error)
