  ...
  getTableSchema(){
    this.getData('schema').subscribe({
      next : (data)=>{
        ...
        this.mainTableList=this.tableList.filter(el=>(el?.type==1 && !el?.tenant))
        this.tenantTable=this.tableList.find(el=>(el?.tenant)) || null
        this.mainTableType=this.mainTableList?.[0]?.name
        if(this.userData?.role!=='user'){
          this.lookupTableList=this.tableList.filter(el=>(el?.type==2 && !el?.tenant))
        }
